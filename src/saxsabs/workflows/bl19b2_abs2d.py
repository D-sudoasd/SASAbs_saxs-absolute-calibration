...
def find_reference_paths(input_root: str | Path) -> ReferencePaths:
...
def load_and_write_mask(
    *,
    ...
    if reference_paths.mask is not None and reference_paths.mask.exists():
...
def write_provenance_package(
    *,
    ...
    summary = {
        ...
        "source_poni_path": str(Path(config.poni_path)),
...
def _write_processing_config(
    ...
    *,
    ...
    lines = [
        ...
        f"source_poni_path: {Path(config.poni_path)}",
...
def run_bl19b2_abs2d(config: BL19B2Abs2DConfig) -> dict[str, Any]:
    ...
    reference_paths = find_reference_paths(input_root)
